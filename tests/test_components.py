...
class TestSCD2Config:
    ...
    def test_config_creation_with_latest_record_flag_enabled(self):
        """Test creating config with latest_record_flag enabled."""
        config = SCD2Config.create(
            business_keys=["id"],
            enable_latest_record_flag=True,
        )

        assert config.enable_latest_record_flag == True
        assert config.business_keys == ["id"]

    def test_config_creation_with_latest_record_flag_disabled(self):
        """Test creating config with latest_record_flag disabled (default)."""
        config = SCD2Config.create(business_keys=["id"])

        assert config.enable_latest_record_flag == False
        assert config.business_keys == ["id"]
...
class TestSCD2Columns:
    ...
    def test_default_column_names(self):
        ...
        assert columns.latest_record_flag == "latest_record_flag"
    ...
    def test_column_list(self):
        """Test getting column list with default values."""
        columns = SCD2Columns()
        column_list = columns.column_list()
        ...
        assert set(column_list) == set(expected_columns)

    def test_column_list_with_custom_names(self):
        """Test getting column list with custom column names."""
        columns = SCD2Columns(
            valid_from="start_date",
            valid_until="end_date",
            latest_record_flag="latest_record_flag",
        )
        column_list = columns.column_list()
